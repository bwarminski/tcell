...
func (t *tScreen) getWinSize() (int, int, error) {
	...
	rows := int(wsz.Row)
	if cols == 0 {
		colsEnv := terminfo.EnvFromOptions("COLUMNS", t.options)
		if colsEnv != "" {
	...
	if rows == 0 {
		rowsEnv := terminfo.EnvFromOptions("LINES", t.options)
		if rowsEnv != "" {
