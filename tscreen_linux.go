...
func (t *tScreen) termioInit() error {
	...
	if t.remote != nil {
		t.in = t.remote.In
		t.out = t.remote.Out
		t.remote.Subscribe(t.sigwinch)
	} else {
		if t.in, e = os.OpenFile("/dev/tty", os.O_RDONLY, 0); e != nil { //TODO - might need goterm here
			goto failed
		}
...
func (t *tScreen) termioFini() {
	...
	if t.out != nil && t.tiosp != nil && t.remote != nil {
		unix.IoctlSetTermios(int(t.out.Fd()), unix.TCSETSF, t.tiosp.tio)
		t.out.Close()
	}

	if t.in != nil && t.remote != nil {
		t.in.Close()
	}
}
...
func (t *tScreen) getWinSize() (int, int, error) {
	if t.remote != nil {
		dims := t.remote.LatestDimensions.Load().(terminfo.RemoteDimensions)
		return dims.Columns, dims.Rows, nil
	}
...
func (t *tScreen) Beep() error {
	t.writeString(string(byte(7))) // TODO: We may need to introspect this somehow
	return nil
}
